...
const Sidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
  ...
  // --- FUNGSI LOGOUT TOTAL (ANTI NYANGKUT & ANTI HANG) ---
  const handleLogout = async () => {
    try {
      // 1. TUTUP SIDEBAR DULU (Penting biar backdrop transparan ilang)
      setIsOpen(false);
      ...
      // 2. Bersihkan session/storage total
      localStorage.clear();
      sessionStorage.clear();

      // 3. SOLUSI JITU: Paksa balik ke Login dan REFRESH aplikasi
      // Kasih delay 100ms biar state Sidebar sempet ketutup sempurna
      setTimeout(() => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-[100] w-72 bg-slate-900 text-slate-300 transform transition-transform duration-500 ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        } lg:translate-x-0 border-r border-slate-800 flex flex-col h-full shadow-2xl lg:shadow-none`}
      >
            ...
            (item) =>
              item.roles.includes(role) && (
                <Link
                  ...
                  to={item.path}
                  onClick={() => setIsOpen(false)}
                  className={`flex items-center justify-between px-4 py-3.5 rounded-2xl font-bold transition-all duration-300 ${
